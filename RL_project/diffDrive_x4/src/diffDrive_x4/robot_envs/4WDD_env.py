...
import rospy
import time
from diffDrive_x4 import robot_gazebo_env
...
class 4WDDEnv(robot_gazebo_env.RobotGazeboEnv):
    """Superclass
    """
    ...
    def __init__(self, ros_ws_abspath):
        ...
        * /vector/laser: Range Readings
        ...
        Args:
        """
        rospy.logdebug("Start 4WDDEnv INIT...")
        ...
        super(4WDDEnv, self).__init__(controllers_list=self.controllers_list,
        ...
        rospy.Subscriber("/4WDD/imu", Imu, self._imu_callback)
        rospy.Subscriber("/4WDD/laser", Range, self._laser_scan_callback)
        ...
        rospy.logdebug("Finished 4WDDEnv INIT...")
    ...
    def _check_imu_ready(self):
        self.imu = None
        rospy.logdebug("Waiting for /4WDD/imu to be READY...")
        while self.imu is None and not rospy.is_shutdown():
            try:
                self.imu = rospy.wait_for_message("/4WDD/imu", Imu, timeout=5.0)
                rospy.logdebug("Current /4WDD/imu READY=>")

            except:
                rospy.logerr("Current /4WDD/imu not ready yet, retrying for getting imu")
    ...
    def _check_laser_scan_ready(self):
        ...
        rospy.logdebug("Waiting for /4WDD/laser to be READY...")
        while self.laser_scan is None and not rospy.is_shutdown():
            try:
                self.laser_scan = rospy.wait_for_message("/4WDD/laser", Range, timeout=1.0)
                rospy.logdebug("Current /4WDD/laser READY=>")

            except:
                rospy.logerr("Current /4WDD/laser not ready yet, retrying for getting laser_scan")
    ...
    def move_base(self, linear_speed, angular_speed, epsilon=0.05, update_rate=10):
        ...
        rospy.logdebug("4WDD Base Twist Cmd>>" + str(cmd_vel_value))
        self._check_publishers_connection()
